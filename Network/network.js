...
export class Network {
    static sendRequest(request, callback) {
        ...
        if (request.data !== null && !(request.data instanceof User|| Task)) {
            callback(JSON.stringify({ success: false, message: "data is of wrong type", data: data }), 400);
        ...
        if(request.method=="GET"){
            ...
            if(type_=="task"){
                data=taskServer.delete_(request.url.split("/")[3])
            }
            if(type_=="user"){
                data=userServer.delete_(request.url.split("/")[3])
            }
        }  
        ...
        setTimeout(() => {
            if (dropChance < 0.3) { // לדוגמה, השמטה בהסתברות של 30%
                callback(JSON.stringify({ success: false, message: "error has occured" , data: data}), 500);
                return;
            }
